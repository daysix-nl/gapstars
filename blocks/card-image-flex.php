            ...
            <?php while( have_rows('card') ): the_row(); ?>
                <div 
                    ...
                    <?php if(get_sub_field('anchors')): ?>
                            id="<?php  strtolower(echo get_sub_field('anchor_name')); ?>"
                    <?php endif; ?>
                >
                    <div
                        <?php if(get_field('column') == 'grid-cols-3'): ?>
                        ...
                        <?php elseif(get_field('column') == 'grid-cols-2'): ?> 
                            class="<?php echo get_field('image_height'); ?> w-full overflow-hidden flex items-center justify-center scale-img "
                        <?php elseif(get_field('column') == 'grid-cols-1'): ?> 
                            class="<?php echo $height ?> w-full md:w-1/2 overflow-hidden flex items-center justify-center scale-img <?php echo get_sub_field('order'); ?> scale-100"
                        <?php endif; ?>
                    >
                        <?php if(get_field('column') == 'grid-cols-3'): ?>
                            <div class="background-image-contact <?php  echo (get_field('border-radius')) ? "rounded-3xl" : "rounded-t-xl"; ?>" style="background-image: url('<?php echo get_sub_field('image'); ?>');"></div>
                        <?php elseif(get_field('column') == 'grid-cols-2'): ?> 
                            <div class="background-image-contact <?php  echo (get_field('border-radius')) ? "rounded-3xl" : "rounded-t-xl"; ?>" style="background-image: url('<?php echo get_sub_field('image'); ?>');"></div>
                        <?php elseif(get_field('column') == 'grid-cols-1'): ?> 
                            <img class="w-full  <?php  echo (get_field('vertical-align')) ? "h-full object-cover" : ""; ?> scale-100 <?php  echo (get_field('border-radius')) ? "rounded-3xl" : ""; ?>" src="<?php echo get_sub_field('image'); ?>" alt="<?php echo get_sub_field('title'); ?>">
                        <?php endif; ?>
                    </div>
                    <div 
                        <?php if(get_field('column') == 'grid-cols-3'): ?>
                    ...
                    >
                        <h3 
                            ...
                            <?php endif; ?>
                        ><?php echo do_shortcode(echo get_sub_field('title')); ?></h3>

                        <?php if(get_sub_field('text')): ?>
                            <p class="<?php echo get_field('text-color_inner'); ?> text-16 leading-26"><?php echo get_sub_field('text'); ?></p>
                        <?php endif; ?>
