            ...
            <?php while( have_rows('card_repeater') ) : the_row(); ?>
                ...
                <div class="<?php echo get_field('background-color_inner'); ?> rounded-3xl w-full h-full px-4 py-2 ">
                        <div class="flex flex-row items-center">
                            <img class="w-[52px] mr-3" src="/wp-content/themes/gapstars/img/icons-acf/<?php echo get_sub_field('card_repeater_icon');?>.svg" alt="">
                            <h3 class="text-22 lg:text-26 leading-30 lg:leading-36 <?php echo get_field('text-color_inner'); ?> "><?php echo get_sub_field('card_repeater_title');?></h3>
                        </div>
                    <?php elseif (get_field('column') == 'grid-cols-2'): ?>
                <div class="<?php echo get_field('background-color_inner'); ?> rounded-3xl w-full h-full px-4 py-6 ">
                        <?php if(get_sub_field('image_icon') == 'icon' ): ?>
                                <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center">
                                    <img class="lg:order-1 ml-3 w-[52px]" src="/wp-content/themes/gapstars/img/icons-acf/<?php echo get_sub_field('card_repeater_icon');?>.svg" alt="">
                            <?php elseif(get_sub_field('image_icon') == 'image' ):  ?>
                                <div class="flex flex-col lg:flex-row lg:items-center">
                                    <img class="w-[100px] h-[100px] object-cover mr-2 rounded-full" src="<?php echo get_sub_field('image'); ?>" alt="">
                            <?php endif;  ?>
                            <h3 class="lg:order-0 text-22 lg:text-26 leading-30 lg:leading-36 <?php echo get_field('text-color_inner'); ?> mt-3 lg:mt-0"><?php echo get_sub_field('card_repeater_title');?></h3>
                        </div>
                    <?php elseif (get_field('column') == 'grid-cols-3'): ?>
                <div class="<?php echo get_field('background-color_inner'); ?> rounded-3xl w-full h-full px-4 py-6 ">
                        <div class="flex flex-col">
                            <img class="w-[52px]" src="/wp-content/themes/gapstars/img/icons-acf/<?php echo get_sub_field('card_repeater_icon');?>.svg" alt="">
                            <h3 class="text-22 lg:text-26 leading-30 lg:leading-36 <?php echo get_field('text-color_inner'); ?> mt-3"><?php echo get_sub_field('card_repeater_title');?></h3>
                        </div>
                    <?php endif; ?>
                        <?php if(get_sub_field('text_input') == 'paragraph'): ?>
                            <?php if(get_sub_field('card_repeater_text')): ?>
                                <p class="text-16 leading-24 lg:text-16 lg:leading-26 <?php echo get_field('text-color_inner'); ?> mt-3"><?php echo get_sub_field('card_repeater_text');?></p>
                            <?php endif; ?>
                        <?php else: ?>
                            <?php if( have_rows('list') ): ?>
                                <ul class="<?php echo get_field('text-color_inner'); ?> flex flex-col gap-1 mt-3">
                                    <?php while( have_rows('list') ): the_row(); ?>
                                        <li class="<?php echo get_field('text-color_inner'); ?> text-16 leading-26 "><?php echo get_sub_field('list_item') ?></li>
                                    <?php endwhile; ?>
                                </ul>
                            <?php endif; ?>
                        <?php endif; ?>
